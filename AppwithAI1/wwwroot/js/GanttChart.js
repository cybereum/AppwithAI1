...
class GanttChart {
    ...
    addActivity(activity) {
        ...
        // Check if the activity id is unique
        if (this.activities.has(activity.id)) {
            throw new Error('Activity ID must be unique');
        }
        // Check if the task ids are unique
        for (let task of activity.tasks) {
            if (this.activities.has(task.id)) {
                throw new Error('Task ID must be unique');
            }
            // Check if the subtask ids are unique
            for (let subtask of task.subtasks) {
                if (this.activities.has(subtask.id)) {
                    throw new Error('Subtask ID must be unique');
                }
            }
        }
    ...
    render() {
        ...
        // Calculate the start and end dates of the chart
        let startDate = this.activities.values().next().value.start;
        let endDate = Array.from(this.activities.values()).reduce((latest, activity) => {
            return activity.end > latest ? activity.end : latest;
        }, startDate);

        // Create a marker for each day
        for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
            let x = (date - startDate) / (24 * 60 * 60 * 1000) * this.dayWidth * this.zoomLevel;

            // Create a line for the marker
            let line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            line.setAttribute('x1', x);
            line.setAttribute('y1', 0);
            line.setAttribute('x2', x);
            line.setAttribute('y2', this.activities.size * this.taskHeight);
            line.setAttribute('stroke', '#d2fbff');
            ...
            let text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            text.setAttribute('x', x);
            text.setAttribute('y', -5); // Position the text above the chart
            text.setAttribute('fill', '#d2fbff');
            text.textContent = date.getDate();
            scaleGroup.appendChild(text);
        }
        // Add the scale group to the SVG element
        this.svg.appendChild(scaleGroup);
