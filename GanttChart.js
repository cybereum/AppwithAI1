...
class GanttChart {
    ...
    handleMouseDown(event) {
        // Get the ID of the activity that was clicked
        ...
        if (activity) {
            // Store the activity and the current mouse position
            this.draggingActivity = activity;
            this.draggingStartX = event.clientX;
        }

        // Check if the user clicked on the left or right edge of an activity to resize it
    ...
    handleMouseMove(event) {
        if (this.draggingActivity) {
            // Calculate the change in days based on the mouse movement
            let dx = event.clientX - this.draggingStartX;
            let deltaDays = Math.round(dx / (this.dayWidth * this.zoomLevel));

            // Update the start and end dates of the activity
            ...
            rect.setAttribute('x', (this.draggingActivity.start - this.activities[0].start) / (24 * 60 * 60 * 1000) * this.dayWidth * this.zoomLevel);
        ...
        if (this.resizingActivity) {
            ...
            // Update the start or end date of the activity
            ...
            // Update the position or size of the rectangle
            let rect = document.getElementById(this.resizingActivity.id);
            if (this.resizingEdge === 'left') {
                rect.setAttribute('x', (this.resizingActivity.start - this.activities[0].start) / (24 * 60 * 60 * 1000) * this.dayWidth * this.zoomLevel);
    ...
    handleDoubleClick(event) {
        // Get the ID of the activity that was double-clicked
        let activityId = event.target.id;
        ...
        if (activity) {
            // Open a dialog to edit the activity details
    ...
    handleClick(event) {
        ...
        if (activity) {
            ...
            if (this.selectedActivity === activity) {
                ...
                // If another activity is selected, deselect it
